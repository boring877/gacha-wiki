...
function analyzeBundle() {
  ...
  console.log(
    `Gzipped: ${formatBytes(totalGzipSize)} (${((totalGzipSize / totalSize) * 100).toFixed(1)}% compression)`
  );
  console.log(
    `Brotli: ${formatBytes(totalBrotliSize)} (${((totalBrotliSize / totalSize) * 100).toFixed(1)}% compression)\n`
  );
  ...
  assets.slice(0, 10).forEach(asset => {
    const compressionInfo = asset.gzipSize ? ` | Gzipped: ${formatBytes(asset.gzipSize)}` : '';
  ...
  if (largeAssets.length > 0) {
    console.log(
      `⚠️  Found ${largeAssets.length} large assets (>100KB). Consider code splitting or lazy loading.`
    );
  }
  ...
  if (uncompressedAssets.length > 0) {
    console.log(
      `⚠️  Found ${uncompressedAssets.length} uncompressed text assets. Run compression.`
    );
  }
