// Morning Sunlight Memory Data - Zone Nova
// Support Memory Card (No specific character)

export const morningSunlightData = {
  // Basic Memory Information
  name: 'Morning Sunlight',
  chineseName: '晨曦的阳光',
  image: '/images/games/zone-nova/memories/Morning-Sunlight .jpg',
  rarity: 'SR',
  character: null, // No specific character associated
  
  // Memory Stats
  stats: {
    hp: 5000,
    attack: 500,
    defense: 500
  },
  
  // Memory Effects
  effects: [
    'Restriction: Only effective for supports',
    'Cost: 5-price awakening',
    'Attack power increased by 20%',
    'All team skill damage increased by 20%'
  ]
};

// SEO Data
export const morningSunlightSEO = {
  title: 'Morning Sunlight - Support Memory Card | Zone Nova',
  description: 'Complete guide for Morning Sunlight memory card in Zone Nova. Learn about its support-specific effects, attack bonuses, and team skill damage enhancement.'
}; 
