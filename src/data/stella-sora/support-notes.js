// Stella Sora Disc Support Notes Data
// Consolidates all disc supportNote data for the support notes database page
...
// Extract unique note materials from all discs
function extractUniqueMaterials(discs) {
  const materials = new Set();
  discs.forEach(disc => {
    if (disc && disc.supportNote) {
      disc.supportNote.forEach(level => {
        level.forEach(mat => {
...
// Process and export disc support notes data sorted by name
export const discSupportNotes = allDiscs
  .filter(disc => disc && disc.supportNote && disc.supportNote.length > 0)
  .map(disc => ({
    id: disc.id,
    name: disc.name,
...
// Export unique materials list for filtering
export const uniqueMaterials = extractUniqueMaterials(allDiscs);

export default discSupportNotes;
