...
// Example structure
export const createExample = ({ id, title, description, calculation, result, notes = [] }) => {
  return {
    id,
    title,
    description,
...
// Tip structure
export const createTip = ({ id, icon, title, description, priority = 'medium' }) => {
  return {
    id,
    icon,
    title,
...
// Table structure
export const createTable = ({ id, headers, rows, highlightedRows = [], caption = '' }) => {
  return {
    id,
    headers,
    rows,
...
export const defenseFormulasData = createDamageMechanicsPage({
  ...
  description:
    'Master the fundamental defense calculations that determine how much damage your characters take in Silver and Blood.',
  sections: [
    createSection({
      id: 'overview',
  ...
  formulas: [
    createFormula({
      ...
      formula:
        'Final Damage = (Attack × Skill Multiplier × 1300) ÷ (Defense + 1300) × (1 - Damage Reduction%)',
      description: 'The complete damage calculation formula used in Silver and Blood',
      variables: [
        {
          name: 'Attack',
          description: "Character's total attack power (base + equipment + buffs)",
        },
        {
          name: 'Skill Multiplier',
          description: 'The multiplier value of the skill being used (e.g., 1.5x, 2.0x)',
        },
        ...
        { name: '1300', description: 'Defense coefficient constant used in Silver and Blood' },
        {
          ...
          description:
            'Additional damage reduction from abilities, expressed as decimal (e.g., 0.2 for 20%)',
        },
      ],
    }),
    createFormula({
  ...
  tips: [
    createTip({
      ...
      description:
        'The constant 1300 determines how effective defense is. When Defense = 1300, you take 50% of base damage.',
      priority: 'high',
    }),
    createTip({
      ...
      description:
        'Higher skill multipliers significantly increase damage output. A 3.0x skill does 50% more damage than a 2.0x skill.',
      priority: 'high',
    }),
    createTip({
      ...
      description:
        'Multiple damage reduction sources often stack multiplicatively, not additively. 20% + 20% ≠ 40%.',
      priority: 'medium',
    }),
    createTip({
      id: 'attack-scaling',
      ...
      description:
        'Attack power scales linearly with damage - doubling attack doubles damage output (before defense).',
      priority: 'medium',
    }),
  ],
});
