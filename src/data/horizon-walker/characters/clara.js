// Clara Character Data - Horizon Walker
...
const rawClaraData = {
  ...
  description: "Clara is an EX rarity hero with a deployment cost of 8, dealing Physical damage.",
...
export const claraData = sanitizeCharacterData(rawClaraData);
export const claraSEO = {
  title: `Clara - Horizon Walker Character Guide`,
  description: `Complete guide for Clara, EX rarity 8 cost hero in Horizon Walker. Skills, traits, profile and team tips.`,
};
...
const rawHighlighting = {
  ...
  skillNames: Object.keys(rawClaraData.skills || {})
    .map(key => rawClaraData.skills[key]?.name)
    .filter(Boolean),
  traitNames: Object.keys(rawClaraData.uniqueTraits || {})
    .map(key => rawClaraData.uniqueTraits[key]?.name)
    .filter(Boolean),
};

export const claraHighlighting = sanitizeHighlightPatterns(rawHighlighting);
