/**
 * MAJO Clock Timer Functionality
...
const MAJO_TIMEZONE_OFFSET = 0; // UTC
const MS_PER_DAY = 1000 * 60 * 60 * 24;
const MS_PER_HOUR = 1000 * 60 * 60;
const MS_PER_MINUTE = 1000 * 60;

class MajoClockTimer {
  constructor() {
  ...
  setupClock() {
    try {
      ...
      this.config = window.majoConfig;
      if (!this.config) {
        console.warn('MAJO config not found');
        return;
      }
      ...
      console.error('Failed to setup MAJO clock:', error);
      this.cleanup();
    }
  }
  ...
  updateCurrentDateTime() {
    ...
    // Update KST time
    if (currentUTC) {
      const kstTime = new Date(now.getTime() + MAJO_TIMEZONE_OFFSET);
      const hours = String(kstTime.getUTCHours()).padStart(2, '0');
      const minutes = String(kstTime.getUTCMinutes()).padStart(2, '0');
      const seconds = String(kstTime.getUTCSeconds()).padStart(2, '0');
      const newTime = `UTC ${hours}:${minutes}:${seconds}`;
