...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let filteredCharacters = Array.from(tableBody.querySelectorAll('tr'));
  ...
  function applyFilters() {
    ...
    filteredCharacters = allRows.filter(row => {
      ...
      const element = row.querySelector('.element-badge')?.textContent || '';
      const role = row.querySelector('.role-badge')?.textContent || '';
      const faction = row.querySelector('.faction-badge')?.textContent || '';
      const trekkerStyle = row.querySelector('.trekker-style-badge')?.textContent || '';
      ...
      if (filters.faction && faction !== filters.faction) {
        return false;
      }

      // Apply trekker style filter
      if (filters.trekkerStyle && trekkerStyle !== filters.trekkerStyle) {
        return false;
      }
  ...
  function resetFilters() {
    ...
    filteredCharacters = Array.from(tableBody.querySelectorAll('tr'));
    renderTable();
  }
