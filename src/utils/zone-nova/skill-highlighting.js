...
export const defaultZNHighlightPatterns = {
  damage:
    /\b(?:\d{1,4}%\s+attack\s+power|holy\s+damage|fire\s+damage|ice\s+damage|wind\s+damage|chaos\s+damage|damage|attack\s+power)\b/gi,
  effects:
    /(?:reduces?\s+enemy.*?by\s+\d+%|increases?\s+.*?by\s+\d+%|grants?\s+.*?for\s+\d+.*?seconds?)/gi,
  buffsDebuffs:
    /\b(?:shield|buff|debuff|heal|healing|recovery|stun|freeze|burn|poison|charm|taunt|weakness|vulnerability|immunity|resistance|regeneration|overflow|critical\s+hit|penetration)\b/gi,
  numbers:
    /\b(?:\d{1,4}%|\d{1,3}\.?\d?\s*s|\d{1,3}\s+seconds?|\d{1,3}\s+rounds?|\d{1,2}\s+turns?|\d{1,2}\s+enemies?|cooldown:\s+\d+\.?\d?\s*s)\b/gi,
  keywords:
    /\b(?:deals?|inflicts?|grants?|restores?|applies?|removes?|blocks?|gains?|loses?|activates?|triggers?|casts?|channels?|summons?|enhances?|reduces?|increases?|lasting|duration|while|when|after|until|once|every|maximum|equal\s+to|up\s+to|designated|target|enemy|ally|allies)\b/gi,
  attributes:
    /\b(?:HP|health|attack|defense|speed|accuracy|evasion|crit\s+rate|crit\s+dmg|AP|MP|energy|holy|fire|ice|wind|chaos|element|elemental|resistance|penetration)\b/gi,
};
